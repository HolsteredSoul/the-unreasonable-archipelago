...
export function recordVictory(records: Achievements, state: GameState): Achievements {
  ...
  if (Object.hasOwn(records, state.seed) && records[state.seed].growth >= growth) return records;
  return { ...records, [state.seed]: { rank: growth >= 6 ? 3 : growth >= 3 ? 2 : 1, growth } };
}
export function loadAchievements(): Achievements {
  try {
    ...
    return Object.fromEntries(Object.entries(parsed).filter(([seed, value]) => seed.length > 0 && seed.length <= 100 && value && Number.isInteger(value.growth) && value.growth >= 0 && value.growth <= 15 && value.rank === (value.growth >= 6 ? 3 : value.growth >= 3 ? 2 : 1)));
  } catch { return {}; }
}
