...
export function TideChart({ state }: { state: GameState }) {
  ...
  return <>
    ...
    {moonwake ? <ol className="tide-chart">
      ...
      <li className={state.tide === 8 ? 'current' : ''}><span>08</span><div><strong>Every bell needs a sheltered way home</strong><p>No drift. Every bell must finish with growth 3, a Heart connection, shelter, and less than 3 stress. Place an island one hex {bearings[upstream]} of each bell, or use a breakwater’s longer wake. Anchors do not stop storm stress.</p></div></li>
