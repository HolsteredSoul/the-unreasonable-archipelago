...
export const ACTION_COPY: Record<ActionName, { title: string; detail: string }> = {
  ...
  whaleTow: { title: 'Whale Tow · 1 action, no food', detail: 'One favour this visit. Moves one hex; the coming current still applies.' },
...
export function balanceChange(before: GameState, after: GameState): string {
  ...
  return parts.length ? parts.join(' · ') : `Food ${after.food} · timber ${after.timber} unchanged`;
}
