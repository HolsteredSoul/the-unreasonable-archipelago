...
export default function App() {
  ...
  const [modal, setModal] = useState<'intro' | 'help' | 'settings' | 'restart' | 'medals' | 'tides' | 'whale' | null>(() => session.seenIntro ? null : 'intro');
  ...
  return <main onPointerDownCapture={event => { unlockMusic(); inputOrigin.current = { x: event.clientX, y: event.clientY }; }} onKeyDownCapture={() => { unlockMusic(); inputOrigin.current = null; }} className={`game-app ${session.settings.reducedMotion ? 'reduced-motion' : ''} ${advancing ? 'is-advancing' : ''} ${seaFocus ? 'sea-focus' : ''} ${focusPanel ? `reveal-${focusPanel}` : ''} ${mode === 'tow' ? 'is-towing' : ''}`}>
    ...
    {modal === 'whale' && <Modal label="How Whale Tow works" onClose={() => setModal(null)} className="guide-modal">
      ...
      <p className="modal-lead"><strong>Costs 1 of your 3 actions. Costs 0 food.</strong> The tow moves your selected island immediately, one hex in the whale’s direction.</p>
      <div className="guide-steps">
